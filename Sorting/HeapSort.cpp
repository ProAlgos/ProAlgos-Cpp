/*
    Heap Sort :
    Uses an array-implemented heap to sort list of numbers.

    Time complexity:
    O(nlog n), where n is the number of elements to be sorted
...
void heapsort(vector<int> &heap, const int size, const int order, const bool showState){
    ...
    while (last>0){
        ...
        n-=1;
        heapify(last,0,n, order, showState);
    }
}
//Makes ordered heap by "heapify"ing from highest indexed non-leaf node(curr_node) to the top node, index[0]
void make_heap(vector<int> &heap, const int size, const int order, cosnt bool showState){
