/*
    ...
    Time complexity is O(n + k), where n is the number of elements in input array
...
void counting_sort(vector<int>& values, const int order, const bool to_show_state) {
    ...
    // start and end indexes for cumulative sum calculation
    int sum_start = 1;
    ...
    // now freq[i] is number of elements in sorted array that less or equal than i
